    ...
    : IRegisterOrderPaymentUseCase
{
    public async Task HandleAsync(OrderPaymentRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            var transactionDetail = new PaymentTransaction(request.TransactionNumber, request.TransactionDocument, request.TransactionMessage);
            ...
            await eventBus.HandleAsync(new OrderPaymentUpdatedEvent(request.PaymentStatus, request.TransactionNumber), cancellationToken);
        }
    }
}